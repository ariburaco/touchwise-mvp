        ...
        },
        {
          onSuccess: (context) => {
            ...
            if ('twoFactorRedirect' in (context.data || {})) {
              // 2FA redirect will be handled by the auth client plugin
              return;
            }
  ...
  const handleSocialLogin = async (provider: 'google' | 'github') => {
    ...
    try {
      await authClient.signIn.social({
        provider,
        callbackURL: returnUrl ? decodeURIComponent(returnUrl) : '/dashboard',
      });
    } catch (error) {
    ...
    try {
      await authClient.signIn.magicLink({
        ...
        callbackURL:
          ...
          (returnUrl ? decodeURIComponent(returnUrl) : '/dashboard'),
      });
